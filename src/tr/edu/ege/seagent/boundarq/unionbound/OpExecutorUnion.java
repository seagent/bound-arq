...
public class OpExecutorUnion extends OpExecutor {
	...
	protected OpExecutorUnion(ExecutionContext execCxt) {
		super(execCxt);
		this.execCxt = execCxt;
		dispatcherBound = new ExecutionDispatchUnion(this);
	}

	/**
	 * {@link SeagentExecutionDispatch} instance.
	 */
	protected ExecutionDispatchUnion dispatcherBound = null;

	// Set this to a different factory implementation to have a different
	// OpExecutor.
	protected static final OpExecutorFactoryUnion factoryBound = new OpExecutorFactoryUnion() {
		public OpExecutorUnion create(ExecutionContext execCxt) {
			return new OpExecutorUnion(execCxt);
		}
	};
	...
	static QueryIterator execute(Op op, QueryIterator qIter,
		...
		OpExecutorUnion exec = createOpExecutor(execCxt);
		QueryIterator q = exec.executeOp(op, qIter);
		return q;
	}

	/**
	 * {@link #dispatcherBound} is used to execute op and input.
	 * 
	...
	public QueryIterator executeOp(Op op, QueryIterator input) {
		level++;
		QueryIterator qIter = dispatcherBound.exec(op, input);
	 ...
	 * @return
	 */
	private static OpExecutorUnion createOpExecutor(ExecutionContext execCxt) {
		OpExecutorFactoryUnion factory = ((ExecutionContextUnion) execCxt)
				.getBoundExecutor();
		if (factory == null)
	...
	protected QueryIterator execute(OpService opService, QueryIterator input) {
		return new QueryIterServiceUnion(input, opService, this.execCxt);
	}
	...
	protected QueryIterator execute(OpUnion opUnion, QueryIterator input) {
		...
		return new QueryIterUnionBoundUnion(input, x, execCxt);
	}
